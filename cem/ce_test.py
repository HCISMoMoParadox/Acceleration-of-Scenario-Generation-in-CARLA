...
def ce_test_carla_baseline(seed_value, rho, alpha, scenario_id):
  ...
  state_checkpoint_root = './checkpoint/' + scenario_id
...
def execute(obj, work_list, worker_state):
  ...
  return obj, work_list, worker_state, work_result

def send_sample_task(seed_value, rho, alpha):
  #  Socket to talk to server
  context = zmq.Context()
  ...
  for i in range(num_iter):
    ...
    while len(work_list) > 0:
      ...
      if worker1_state == 1:
        ...
        j += 1 
        obj, work_list, worker1_state, work_result = execute(obj, work_list, worker1_state)      
      if worker2_state == 1:
        param1 = sample_wp[0][j]
        param2 = sample_w[0][j]
        sender.send_string(("%d,%f,%d" % (param1, param2, j)))
        worker2_state = 0
        j += 1
        obj, work_list, worker2_state, work_result = execute(obj, work_list, worker2_state)
      time.sleep(2)
...
def ce_test_carla_para(seed_value, rho, alpha):
  ...
  for i in range(num_iter):
    ...
    # if i==1:
    print('cem time:', time.time() - start)
  ############################ end cem loop ############################
