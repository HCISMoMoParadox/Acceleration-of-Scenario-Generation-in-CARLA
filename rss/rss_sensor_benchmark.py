...
# -- RssSensor -----------------------------------------------------------------
# ==============================================================================
LateralRelativePosition = ['AtLeft', 'OverlapLeft', 'Overlap', 'OverlapRight',
  'AtRight']
LongitudinalRelativePosition = ['InFront', 'OverlapFront', 'Overlap', 'OverlapBack',
  'AtBack']
SituationType = ['NotRelevant' , 'SameDirection', 'OppositeDirection', 'IntersectionEgoHasPriority',
  'IntersectionObjectHasPriority', 'IntersectionSamePriority', 'Unstructured']
...
class RssSensor(object):
    ...
    def destroy(self):
        if self.sensor:
            ...
            # saving predictions
            with open(os.path.join(os.path.abspath(os.getcwd()), self.stored_path, 'rss_predictoins.json'), 'w') as f:
            # with open('rss_predictoins.json', 'w') as f:
                json.dump(self.predictions, f, indent=4)
            print("RSS predictions saved")
    ...
    def _on_rss_response(self, response):
        ...
        if delta_time > -0.05:
            # print('timestamp: %f' % (response.timestamp))
            ...
            # if self.response_valid and not response.proper_response.isSafe:
