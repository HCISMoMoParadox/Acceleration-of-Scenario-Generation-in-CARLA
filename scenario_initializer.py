...
try:
    ...
    sys.path.append('../carla/dist/carla-0.9.13-py3.7-linux-x86_64.egg')
    #
...
class CarlaEnv(object):
    def __init__(self, start_index, degree_deff):
        ...
        try:
            ...
            lights = self.sim_world.get_actors().filter('traffic.traffic_light*')
...
def run_scenario_with_params(params):
    env_init = 0
    env_step = 0
    env_TTC = 0
    env_end = 0
    time_last = time.time()
    env = CarlaEnv(params[0], params[1])
    time_end = time.time()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        result = "87"
        if len(params) == 3:
            result = run_scenario_with_params(params[:2])
