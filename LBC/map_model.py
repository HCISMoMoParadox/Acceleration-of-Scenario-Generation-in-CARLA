...
class MapModel(pl.LightningModule):
    ...
    def forward(self, topdown, target, debug=False):
        ...
        reshaped = torch.cat((topdown, target_heatmap), 0)
        # print('reshaped', reshaped.shape)
        # print(reshaped.shape)
        # out = self.net(torch.cat((topdown, target_heatmap), 1))
        # print(reshaped.shape)
        out = self.net(reshaped[None, :].cuda())
        # print('out', out.shape)
    ...
    #     target_heatmap = self.to_heatmap(target, topdown)[None, :]
    #     print('target_heatmap.shape', target_heatmap.shape)
    #     out = self.net(torch.cat((topdown, target_heatmap), 0)[None, :])
