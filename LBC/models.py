...
class SegmentationModel(torch.nn.Module):
    ...
    def forward(self, input, heatmap=False):
        if self.hack:
            input = torch.nn.functional.interpolate(input, scale_factor=0.5, mode='bilinear', recompute_scale_factor=True)

        x = self.norm(input)
        x = self.network(x)['out']

        if self.hack:
            x = torch.nn.functional.interpolate(x, scale_factor=2.0, mode='bilinear', recompute_scale_factor=True)
